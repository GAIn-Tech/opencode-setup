...
describe('Rate Limiting', () => {
  ...
  test('should use separate counters for different IPs', async () => {
    ...
    // IP 1 uses its quota
    expect(rateLimit('test:192.168.1.1', limit, windowMs).allowed).toBe(true);
    expect(rateLimit('test:192.168.1.1', limit, windowMs).allowed).toBe(true);
    expect(rateLimit('test:192.168.1.1', limit, windowMs).allowed).toBe(false);
    
    // IP 2 should have its own quota
    expect(rateLimit('test:192.168.1.2', limit, windowMs).allowed).toBe(true);
    expect(rateLimit('test:192.168.1.2', limit, windowMs).allowed).toBe(true);
    expect(rateLimit('test:192.168.1.2', limit, windowMs).allowed).toBe(false);
  });
