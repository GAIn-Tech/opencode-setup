...
test('listBudgetSessions and getModelBudgets handlers return structured content', async () => {
  const governor = {
    getAllSessions() {
      return {
        ses_a: {
          'anthropic/claude-sonnet-4-5': { used: 1000, remaining: 199000, max: 200000, pct: 0.005, status: 'ok' },
        },
      };
    },
  };
  ...
  assert.ok(budgets.structuredContent.budgets['anthropic/claude-sonnet-4-5']);
});
...
test('handlers return MCP error payloads on exceptions', async () => {
  ...
  const handlers = createContextGovernorHandlers(governor);
  const budgetResult = await handlers.getContextBudgetStatus({ sessionId: 'ses_err', model: 'anthropic/claude-sonnet-4-5' });
