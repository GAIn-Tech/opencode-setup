...
export async function injectBoulderContinuation(input: {
  ...
  const preferredSessionContext = preferredTaskSessionId
	...
	const continuationAgent = resolveRegisteredAgentName(
		agent ?? sessionAgent ?? (isAgentRegistered("atlas") ? "atlas" : undefined),
	)

	if (!agent && sessionAgent) {
		log(`[${HOOK_NAME}] Using session agent for continuation`, {
			sessionID,
			sessionAgent,
		})
	} else if (!agent && !sessionAgent) {
		log(`[${HOOK_NAME}] Falling back to atlas continuation agent`, {
			sessionID,
		})
	}
    ...
    const persistedSessionModel = getSessionModel(sessionID)
    const launchModel = persistedSessionModel
      ? { providerID: persistedSessionModel.providerID, modelID: persistedSessionModel.modelID }
      : promptContext.model
    ...
    if (persistedSessionModel) {
      log(`[${HOOK_NAME}] Using persisted session model for continuation`, {
        sessionID,
        model: `${persistedSessionModel.providerID}/${persistedSessionModel.modelID}`,
      })
    }
