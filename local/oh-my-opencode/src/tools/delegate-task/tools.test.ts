...
describe("sisyphus-task", () => {
  ...
  describe("DEFAULT_CATEGORIES", () => {
    ...
    test("unspecified-high category uses gpt-5.4 medium as primary", () => {
      // given
  ...
  describe("category variant", () => {
    ...
    test("DEFAULT_CATEGORIES explicit high model passes to background WITHOUT userCategories", async () => {
      ...
      // when - unspecified-high uses gpt-5.4 medium in DEFAULT_CATEGORIES
      await tool.execute(
        {
      ...
      // then - gpt-5.4 should be passed with medium variant
      expect(launchInput.model).toEqual({
        providerID: "openai",
        modelID: "gpt-5.4",
        variant: "medium",
    ...
    test("#given explicit run_in_background=false #when executing #then sync execution succeeds", async () => {
      ...
      const mockClient = {
        ...
        session: {
          ...
          messages: async () => ({ data: [{ info: { role: "assistant" }, parts: [{ type: "text", text: "Done with sync task" }] }] }),
      ...
      expect(result).toContain("Done with sync task")
    }, { timeout: 10000 })
  ...
  describe("modelInfo detection via resolveCategoryConfig", () => {
    ...
    test("default model is used for category with default entry", () => {
      ...
      expect(resolved).not.toBeNull()
      expect(resolved!.config.model).toBe("openai/gpt-5.4-mini")
    })
