...
function sampleBetaInteger(alpha: number, beta: number): number {
  const x = sampleGammaInteger(alpha)
  const y = sampleGammaInteger(beta)
  return x / (x + y)
}

function chooseThompsonCandidate(candidates: string[]): string | undefined {
  ...
  for (let index = 0; index < candidates.length; index += 1) {
    // Bias toward the primary (index 0), while preserving exploration across fallbacks.
    const score = index === 0
      ? sampleBetaInteger(3, 2)
      : sampleBetaInteger(2, 3)
    if (score > bestScore) {
      bestScore = score
...
Available categories: ${allCategoryNames}`,
  ...
  const configuredFallbackModels = resolved.config.fallback_models ?? resolved.config.fallbacks
  const normalizedConfiguredFallbackModels = normalizeFallbackModels(configuredFallbackModels)
  ...
  let explorationSelectedFromFallback = false
  ...
  if (!requirement) {
    ...
    const normalizedFallbackStrings = flattenToFallbackModelStrings(normalizedConfiguredFallbackModels) ?? []
    const routingCandidates = [resolved.model, ...normalizedFallbackStrings].filter(
      (value, index, arr): value is string => Boolean(value) && arr.indexOf(value) === index,
    )
    const shouldUseThompsonSampling =
      resolved.config.routing === "thompson-sampling" &&
      !explicitCategoryModel &&
      !overrideModel &&
      routingCandidates.length > 1

    const selectedExplorationModel = shouldUseThompsonSampling
      ? chooseThompsonCandidate(routingCandidates)
      : undefined

    const primaryCategoryModelForResolution = selectedExplorationModel ?? resolved.model
    const explorationFallbackPool = selectedExplorationModel
      ? routingCandidates.filter((candidate) => candidate !== selectedExplorationModel)
      : normalizedFallbackStrings

    explorationSelectedFromFallback = Boolean(selectedExplorationModel && selectedExplorationModel !== resolved.model)

    const resolution = resolveModelForDelegateTask({
      userModel: explicitCategoryModel ?? overrideModel,
      userFallbackModels: explorationFallbackPool,
      categoryDefaultModel: primaryCategoryModelForResolution,
    ...
    if (resolution && "skipped" in resolution) {
      ...
      fallbackEntry = resolvedFallbackEntry
      matchedFallback = resolvedMatchedFallback === true || explorationSelectedFromFallback
