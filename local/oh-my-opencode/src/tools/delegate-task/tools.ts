  ...
  **WHEN TO USE session_id:**
  ...
  return tool({
    ...
    async execute(args: DelegateTaskArgs, toolContext) {
      ...
      // Workaround for Claude Code bug #17011: background tasks produce empty output
      // If category has force_sync: true, override run_in_background
      const categoryConfig = args.category ? options.userCategories?.[args.category] : undefined
      const forceSync = categoryConfig?.force_sync === true
      const effectiveRunInBackground = forceSync ? false : runInBackground

      if (forceSync) {
        log("[task] force_sync enabled for category, overriding run_in_background=true to sync mode", {
          category: args.category,
        })
      }
      ...
      if (args.category) {
        ...
        if (isUnstableAgent && isRunInBackgroundExplicitlyFalse) {
          const unstableSystemContent = buildSystemContent({
            skillContent,
            skillContents,
          ...
          return executeUnstableAgentTask(args, ctx, options, parentContext, agentToUse, categoryModel, unstableSystemContent, actualModel)
        }
      } else {
      ...
      // Use effectiveRunInBackground to respect force_sync workaround
      if (effectiveRunInBackground) {
