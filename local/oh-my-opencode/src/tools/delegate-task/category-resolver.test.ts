...
describe("resolveCategoryExecution", () => {
	...
	test("does not inherit parent UI model for category delegation when category model is not explicitly set", async () => {
		//#given
		const cacheSpy = spyOn(connectedProvidersCache, "readProviderModelsCache").mockReturnValue({
			models: { openai: ["gpt-5.4-preview", "gpt-5.4-mini"] },
			connected: ["openai"],
		...
		//#when
		const result = await resolveCategoryExecution(args, executorCtx, inheritedModel, "anthropic/claude-sonnet-4-6")
	...
	test("uses thompson-sampling routing for delegated category model exploration", async () => {
		//#given
		const cacheSpy = spyOn(connectedProvidersCache, "readProviderModelsCache").mockReturnValue({
			models: { openai: ["gpt-5.4-mini", "gpt-5.4-preview"] },
			connected: ["openai"],
			updatedAt: "2026-03-03T00:00:00.000Z",
		})
		const agentsSpy = spyOn(connectedProvidersCache, "readConnectedProvidersCache").mockReturnValue(["openai"])
		...
		const randomSpy = spyOn(Math, "random").mockImplementation(() => randomSequence.shift() ?? 0.5)
		...
		executorCtx.userCategories = {
			quick: {
				...
				fallback_models: ["openai/gpt-5.4-preview"],
			},
		}

		//#when
		const result = await resolveCategoryExecution(args, executorCtx, "openai/gpt-5.4", "anthropic/claude-sonnet-4-6")
	...
	test("does not apply thompson-sampling when category model is explicitly user-pinned", async () => {
		//#given
		const cacheSpy = spyOn(connectedProvidersCache, "readProviderModelsCache").mockReturnValue({
			models: { openai: ["gpt-5.4-mini", "gpt-5.4-preview"] },
			connected: ["openai"],
		...
		const agentsSpy = spyOn(connectedProvidersCache, "readConnectedProvidersCache").mockReturnValue(["openai"])
		const randomSpy = spyOn(Math, "random").mockImplementation(() => 0.01)
