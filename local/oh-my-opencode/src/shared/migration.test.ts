...
describe("migrateConfigFile with backup", () => {
  ...
  test("migrates legacy categories.fallbacks to categories.fallback_models", () => {
    // given: config with legacy `fallbacks` key
    const testConfigPath = "/tmp/test-config-category-fallbacks-migration.json"
    const rawConfig: Record<string, unknown> = {
      categories: {
        quick: {
    ...
    // then: legacy alias is migrated to canonical key
    expect(needsWrite).toBe(true)
    const categories = rawConfig.categories as Record<string, Record<string, unknown>>
    expect(categories.quick.fallback_models).toEqual(["openai/gpt-5.4", "google/gemini-3-flash-preview"])
    expect(categories.quick.fallbacks).toBeUndefined()
  })
