...
function migrateCategoryFallbackAliases(categories: Record<string, unknown>): { migrated: Record<string, unknown>; changed: boolean } {
  let changed = false
  const migrated: Record<string, unknown> = { ...categories }

  for (const [categoryName, rawCategoryConfig] of Object.entries(categories)) {
    if (!rawCategoryConfig || typeof rawCategoryConfig !== "object" || Array.isArray(rawCategoryConfig)) {
      continue
    }

    const categoryConfig = rawCategoryConfig as Record<string, unknown>
    if (categoryConfig.fallbacks !== undefined && categoryConfig.fallback_models === undefined) {
      const { fallbacks: legacyFallbacks, ...rest } = categoryConfig
      migrated[categoryName] = {
        ...rest,
        fallback_models: legacyFallbacks,
      }
      changed = true
      continue
    }
    ...
    if (categoryConfig.fallbacks !== undefined && categoryConfig.fallback_models !== undefined) {
      const { fallbacks: _legacyFallbacks, ...rest } = categoryConfig
      migrated[categoryName] = rest
      changed = true
    }
  }
...
export function migrateConfigFile(
  ...
  if (copy.categories && typeof copy.categories === "object") {
    const categoryAliasMigration = migrateCategoryFallbackAliases(copy.categories as Record<string, unknown>)
    if (categoryAliasMigration.changed) {
      copy.categories = categoryAliasMigration.migrated
      needsWrite = true
      log("Migrated legacy category fallback aliases to fallback_models")
    }
